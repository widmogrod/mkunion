...
# moq is a `tool` dependency in go.mod, so there is nothing to install.
# This only warms the build cache and fails early if the tool cannot be built.
(cd "$project_root" && go tool moq -version >/dev/null) || { echo >&2 "cannot build moq. Aborting.
   Check the 'tool' lines in go.mod, and that your Go is 1.24 or newer."; exit 1; }
