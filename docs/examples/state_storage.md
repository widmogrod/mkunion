...
in-memory, DynamoDB, and OpenSearch. All of them are verified against the
same behavioural specification (`x/storage/schemaless/spec`); the tables
below are generated by that test suite.
