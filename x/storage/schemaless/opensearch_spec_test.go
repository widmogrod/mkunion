...
func TestOpenSearchRepositorySpec(t *testing.T) {
	...
	warmup := schemaless.NewOpenSearchRepository[spec.Data](client, indexName)
	command := schemaless.Save(schemaless.Record[spec.Data]{
		ID: "warmup", Type: "spec-warmup", Data: spec.Data{Name: "Warmup", Age: 1},
	})
	...
	spec.RunRepositorySpec(t,
		func(t *testing.T) spec.Repo {
			...
			return schemaless.NewOpenSearchRepository[spec.Data](client, indexName)
		},
...
func TestOpenSearchComplexQuerySpec(t *testing.T) {
	...
	warmup := schemaless.NewOpenSearchRepository[spec.Vehicle](client, indexName)
	command := schemaless.Save(schemaless.Record[spec.Vehicle]{
		ID: "warmup", Type: "spec-warmup",
		Data: spec.Vehicle{Name: "warmup", Wheels: 1, Engine: &spec.Petrol{Brand: "warmup", Cylinders: 1}},
	})
	command.UpdatingPolicy = schemaless.PolicyOverwriteServerChanges
	_, err := warmup.UpdateRecords(command)
	...
	spec.RunComplexQuerySpec(t,
		func(t *testing.T) spec.ComplexRepo {
			return schemaless.NewOpenSearchRepository[spec.Vehicle](client, indexName)
		},
		openSearchCapabilities(),
	)
}
