...
func TestPart5_traceDiffFindsABehaviourRegression(t *testing.T) {
	...
	live.FS = withConfig
	var v1 []Effect
	out1, err := Run(context.Background(), Trace(EffectHandlerFunc(live), &v1), Notify("name.txt", to))
	require.NoError(t, err)

	live, _, _ = newWorld()
	live.FS = withConfig
	var v2 []Effect
	out2, err := Run(context.Background(), Trace(EffectHandlerFunc(live), &v2), notifyV2("name.txt", to))
	require.NoError(t, err)
...
func TestPart5_policyGuardIsAuthorizationOnEffects(t *testing.T) {
	...
	_, err := Run(context.Background(), Guard(Trace(EffectHandlerFunc(live), &trace), dryRun), Notify("name.txt", to))
	...
	_, err = Run(context.Background(), Guard(EffectHandlerFunc(live), dryRun), Prog(func(fx Fx) (string, error) {
...
func TestPart5_spansForEveryOperationFromOnePlace(t *testing.T) {
	...
	_, err := Run(context.Background(), Spans(EffectHandlerFunc(live), tick, &spans), Notify("name.txt", to))
	...
	_, err = Run(context.Background(), Spans(FailEvery(EffectHandlerFunc(live), 1, blip), tick, &spans), Notify("name.txt", to))
