...
import { ExecutionsTableRefactored } from '../components/tables/ExecutionsTableRefactored'
import { PageHeader } from '../components/layout/PageHeader'
import { TableLoadState } from '../components/tables/TablesSection'
import { useExecutionFilterSync } from '../hooks/useFilterSync'

export function ExecutionsPageRefactored() {
  const { listStates, error } = useWorkflowApi()
  const { executionsRefreshTrigger } = useRefreshStore()
  
  // Use the filter sync hook to automatically sync filters with URL
  const { runIdFilter, scheduleFilter } = useExecutionFilterSync()
  
  console.log('ExecutionsPageRefactored render:', {
    runIdFilter,
    scheduleFilter
  })
